...
export function useRole(tableRef: Ref) {
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      cellRenderer: ({ row, props }) => (
        <el-tag
          size={props.size}
          type={row.sendType === 0 ? "danger" : "success"}
          effect="dark"
        >
          {row.sendType === 0 ? "模糊" : "精确"}
        </el-tag>
      )
    },
    {
    ...
    },
    {
      ...
      minWidth: 100,
      cellRenderer: ({ row }) => {
        return <span style={{ color: "blue" }}>{row.group}</span>;
      }
    },
    {
      label: "修改时间",
  ...
  function onChange({ row, index }) {
    ElMessageBox.confirm(
      ...
      .then(() => {
        ...
        UpdateFenqun_status(row.id).then(res => {
          if (res.code === 200) {
            setTimeout(() => {
              switchLoadMap.value[index] = Object.assign(
                {},
  ...
  function handleDelete(row) {
    deleteFenqunApi(row.id).then(async res => {
      if (res.code === 200) {
  ...
  function onbatchDel() {
    ...
    manyDeleteFenqunApi(getKeyList(curSelected, "id")).then(async res => {
      if (res.code === 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getFenqunData(toRaw(form));
