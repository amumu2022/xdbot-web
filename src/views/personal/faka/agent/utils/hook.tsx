...
export function useRole(tableRef: Ref) {
  ...
  function handleDelete(row) {
    deleteUserApi(row.id).then(async res => {
      if (res.code === 200) {
  ...
  function onbatchDel() {
    ...
    manyDeleteUserApi(getKeyList(curSelected, "id")).then(async res => {
      if (res.code === 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getUserData(toRaw(form));
  ...
  function openDialog(title = "新增", row?: FormItemProps) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          message(`您${title}了用户为${curData.user_id}的这条数据`, {
            type: "success"
          });
          done(); // 关闭弹框
          onSearch(); // 刷新表格数据
