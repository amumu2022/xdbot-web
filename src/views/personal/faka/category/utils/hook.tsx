...
export function useRole(tableRef: Ref) {
  ...
  function onChange({ row, index }) {
    ElMessageBox.confirm(
      ...
      .then(() => {
        ...
        UpdateCategory_status(row.id).then(res => {
          if (res.code === 200) {
            setTimeout(() => {
              switchLoadMap.value[index] = Object.assign(
                {},
  ...
  function handleDelete(row) {
    deleteCategoryApi(row.id).then(async res => {
      if (res.code === 200) {
        message(`您删除了分类ID为${row.id}的这条数据`, {
          type: "success"
        });
        await onSearch();
      } else {
  ...
  function onbatchDel() {
    ...
    manyDeleteCategoryApi(getKeyList(curSelected, "id")).then(async res => {
      if (res.code === 200) {
        message(`已删除分类编号为 ${getKeyList(curSelected, "id")} 的数据`, {
          type: "success"
        });
        await onSearch();
      } else {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getCategoryData(toRaw(form));
  ...
  function openDialog(title = "新增", row?: FormItemProps) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          message(`您${title}了分类为${curData.name}的这条数据`, {
            type: "success"
          });
          done(); // 关闭弹框
          onSearch(); // 刷新表格数据
        ...
        FormRef.validate(valid => {
          if (valid) {
            ...
            if (title === "新增") {
              ...
              UpdateCategory(row?.id, curData).then(async res => {
                if (res.code === 200) {
                  await chores();
                } else {
                  message(`操作失败，${res.message}`, { type: "error" });
